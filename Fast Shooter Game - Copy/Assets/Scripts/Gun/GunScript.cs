...
public class GunScript : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Input.GetButton("Fire2") && !isReloading)
        {
            ...
            if (!hasOptic)
            {
                ...
                if (Lens.localPosition != lensTargetPosition)
            ...
            {
                gunPosition.localPosition = Vector3.Lerp(gunPosition.localPosition,
                    opticAimPosition, aimSpeed * Time.deltaTime);
            }
        ...
        if (Input.GetButton("Fire1") && !outOfAmmo && Time.time >= timeTillFire && canFire && fireMode == "FULL AUTO")
            FireWeapon();
        else if (Input.GetButtonDown("Fire1") && !outOfAmmo && canFire && fireMode == "SINGLE FIRE")
            FireWeapon();
        ...
        if (fireMode == "SINGLE FIRE" && Input.GetKeyDown(KeyCode.B) && fullAutoCapable)
            fireMode = "FULL AUTO";
        else if (fireMode == "FULL AUTO" && Input.GetKeyDown(KeyCode.B))
    ...
    private void FireWeapon()
	{
        ...
        if (slidePosition != null)
            slidePosition.localPosition = targetSlidePosition;
